...
export default function VotesRoot() {
  ...
  const resultsElectionCurrentID =
    resultsElectionRaceCurrentID && resultsElectionRaceCurrentID.includes("##") ? resultsElectionRaceCurrentID.split("##")[0] : null;
  ...
  const resultsElectionPreviousID =
    resultsElectionRacePerviousID && resultsElectionRacePerviousID.includes("##") ? resultsElectionRacePerviousID.split("##")[0] : null;
  const resultsRacePreviousID =
    resultsElectionRacePerviousID && resultsElectionRacePerviousID.includes("##") ? resultsElectionRacePerviousID.split("##")[1] : null;
  ...
  const activeDataPoint = useMemo(() => {
    ...
    if (source && source.properties) return source;
    if (source && allElectionData.has(source)) return { properties: allElectionData.get(source) };
    if (county && allElectionData.has(county))
      // won't work yet because we get precinct level data
