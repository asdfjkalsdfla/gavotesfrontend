...
const convertD3ColorToArray = (color) => {
    return color.replace("rgb(", "").replace(")", "").split(",").map(val => parseInt(val));
}
...
export default function VotesMap({
    ...
    const updateZoomLevel = (viewState) => {
        ...
        if (!userHasSetLevel)
            updateIsCountyLevel(viewState.zoom <= transitionZoom);
    };
    ...
    stats.partyTurnoutMax = Math.max(stats.republicanTurnoutVs2018Max, stats.republicanTurnoutVs2016Max, stats.democratTurnoutVs2016Max, stats.democratTurnoutVs2018Max)
    ...
    switch (elevationApproach) {
        ...
        case "votes":
            elevationFunction = (f) => {
                return f.properties["2020TotalVoters"] * (isCountyLevel ? 0.2 : 2) || 0
            }
            break;
        case "votesYest":
            elevationFunction = (f) => {
        ...
        case "votesYestPer2018":
            elevationFunction = (f) => {
                return f.properties["absVotesYesterday"] / f.properties["2018TotalVoters"] * (isCountyLevel ? 50000 : 10000)
            }
            break;
        case "vs2018Abs":
            elevationFunction = (f) => {
                const value =
                    normalizeZeroOne(f.properties.turnoutAbsSameDayVs2018, stats.turnoutAbsSameDayVs2018Min, stats.turnoutAbsSameDayVs2018Max) * 30000 / (1.66 ** (currentZoomLevel - 7))
                    + 0 || 0;
                return value;
            }
            break
        case "absRemaining2020": // hidden for now
            elevationFunction = (f) => {
                const value =
                    normalizeZeroOne(f.properties.absVotesRemaining, 0, 7000) * 30000 / (1.66 ** (currentZoomLevel - 7))
                    + 0 || 0;  
                return value;
            }
            break;
        case "turnoutVs2016":
            elevationFunction = (f) => {
                const value =
                    normalizeZeroOne(f.properties.turnoutVs2016, stats.turnoutVs2016Min, stats.turnoutVs2016Max) * 30000 / (1.66 ** (currentZoomLevel - 7))
        ...
        default:
            elevationFunction = (f) => {
                const value =
                    normalizeZeroOne(f.properties.turnoutVs2018, stats.turnoutVs2018Min, stats.turnoutVs2018Max) * 30000 / (1.66 ** (currentZoomLevel - 7))
                    + 0 || 0;
                return value;
            }
    }
    ...
    switch (colorApproach) {
        ...
        case "turnoutVs2018":
            colorFunction = (f) => {
                const value = normalizeZeroOne(f.properties.turnoutVs2018, stats.turnoutVs2018Min, stats.turnoutVs2018Max);
        ...
        case "republicanTurnoutVs2018":
            colorFunction = (f) => {
                const value = normalizeZeroOne(f.properties.republicanTurnoutVs2018, 0, stats.partyTurnoutMax);
                const color = d3ScaleChromatic.interpolateGreens(value);
                return convertD3ColorToArray(color);
            }
            break
        case "republicanTurnoutVs2016":
            colorFunction = (f) => {
                const value = normalizeZeroOne(f.properties.republicanTurnoutVs2016, 0, stats.partyTurnoutMax);
                const color = d3ScaleChromatic.interpolateGreens(value);
                return convertD3ColorToArray(color);
            }
            break
        case "democratTurnoutVs2016":
            colorFunction = (f) => {
                const value = normalizeZeroOne(f.properties.democratTurnoutVs2016, 0, stats.partyTurnoutMax);
                const color = d3ScaleChromatic.interpolateGreens(value);
                return convertD3ColorToArray(color);
            }
            break
        case "democratTurnoutVs2018":
            colorFunction = (f) => {
                const value = normalizeZeroOne(f.properties.democratTurnoutVs2018, 0, stats.partyTurnoutMax);
                const color = d3ScaleChromatic.interpolateGreens(value);
                return convertD3ColorToArray(color);
            }
            break
        ...
        case "turnoutAbsSameDayVs2018":
            colorFunction = (f) => {
                const value = normalizeZeroOne(f.properties.turnoutAbsSameDayVs2018, stats.turnoutAbsSameDayVs2018Min, stats.turnoutAbsSameDayVs2018Max)
                const color = d3ScaleChromatic.interpolateGreens(value);
                return convertD3ColorToArray(color);
            }
            break
        case "turnout2018Vs2016":
            colorFunction = (f) => {
                const value = normalizeZeroCenterToZeroOne(f.properties.turnout2018Vs2016 - 1, stats.turnout2018Vs2016Min - 1, stats.turnout2018Vs2016Max - 1)
        ...
        case "shift2018To2016":
            colorFunction = (f) => {
                const perAdjusted =
                    normalizeZeroCenterToZeroOne(f.properties["shift2018To2016"], stats.shift2018To2016Min, stats.shift2018To2016Max);
        ...
        case "shift2020To2016":
            colorFunction = (f) => {
                const perAdjusted =
                    normalizeZeroCenterToZeroOne(f.properties["shift2020To2016"], stats.shift2020To2016Min, stats.shift2020To2016Max);
                return COLOR_SCALE(perAdjusted);
            }
            break
        case "shift2020To2018":
            colorFunction = (f) => {
                const perAdjusted =
                    normalizeZeroCenterToZeroOne(f.properties["shift2020To2018"], stats.shift2020To2018Min, stats.shift2020To2018Max);
                return COLOR_SCALE(perAdjusted);
            }
            break
    ...
    // ************************************************
    const layers = [];
    if (colorApproach === "marginShift2020To2016" || colorApproach === "marginShift2020To2018") {
        const layerDot = new ScatterplotLayer({
            ...
            getRadius: f => Math.sqrt(Math.abs((colorApproach === "marginShift2020To2016" ? f.marginShift2020To2016 : f.marginShift2020To2018) * (isCountyLevel ? 250 : 20))),
            getFillColor: f => {
                return (colorApproach === "marginShift2020To2016" ? f.marginShift2020To2016 : f.marginShift2020To2018) > 0 ? [170, 57, 57] : [17, 62, 103];
            },
    ...
    return (<DeckGL
        ...
        getTooltip={({ object }) => {
            ...
            if (lookup[colorApproach]) return ({
                html: `\
            <div>${(colorApproach === "marginShift2020To2016" || colorApproach === "marginShift2020To2018") ?
                        numberFormat.format(lookup[colorApproach]) : numberFormatPercent.format(lookup[colorApproach])}</div>
        `
            }
            )
        }}
