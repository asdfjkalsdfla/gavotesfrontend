...
export default function PrecinctsResultToShapeMatch() {
    ...
    const loadData = async () => {
      ...
      precinctResultsMap.data.forEach((precinct) => {
        electionPrecinctMap.set(
          `${precinct.county}_${precinct.electionResultsPrecinctName}`.toUpperCase(),
          precinct
        )
      }
      );
      ...
      geoJSON.features.forEach((precinct) =>
        shapePrecinctMap.set(
          `${precinct.properties.CTYNAME}_${precinct.properties.PRECINCT_I}`.toUpperCase(),
          precinct
        )
      );
      ...
      geoJSON.features.forEach((precinct) =>
        countiesSet.add(precinct.properties.CTYNAME)
      );
  ...
  useEffect(() => {
    ...
    mapPrecinctsInSelectedCounty.forEach(shapePrecinct => {
      const shapesInElectionMap = electionPrecinctsInSelectedCounty.filter(electionToMapRecord => electionToMapRecord.precinct.toUpperCase() === shapePrecinct.PRECINCT_I)
      if (shapesInElectionMap.length === 0)
        tmpPrecinctsMapNotUsed.push(shapePrecinct);
      if (shapesInElectionMap.length > 1)
        tmpPrecinctsMapUsedMulti.push(shapePrecinct);
    });
  ...
  const precinctOptions = mapPrecinctsInSelectedCounty && mapPrecinctsInSelectedCounty.map((precinctShape) => (
    <Option
      id={precinctShape.PRECINCT_I}
  ...
  const columns = [
    ...
    },
    {
      title: 'Manual',
      dataIndex: 'man',
      render: (text, precinct) => (<Checkbox checked={manualElectionResultsPrecinctsToShapeMap.has(`${precinct.county}_${precinct.electionResultsPrecinctName}`.toUpperCase())} enabled={false} />)
    },
    {
      title: 'Precinct',
      dataIndex: 'precinct',
      render: (text, precinct) => (<Select
        ...
        onChange={(value) => {
          const recordMap = { county: precinct.county, precinct: value, mapPrecinctName: "TODO", electionResultsPrecinctName: precinct.electionResultsPrecinctName, score: 85 }
  ...
  return (
    <div>
      ...
      </Select> <Checkbox onChange={(e) => { updateShowAllCounties(e.target.checked) }} checked={showAllCounties}>All</Checkbox>
      <br />
      <br />
      <b>Not Used:</b> {mapPrecinctsNotSelectedInCounty &&
        mapPrecinctsNotSelectedInCounty.map(shape => <span>{shape.PRECINCT_N} ({shape.PRECINCT_I}), </span>)}
      <br />
      <b>Used Multiple Times:</b> {mapPrecinctsSelectedMultipleTimesInCounty &&
        mapPrecinctsSelectedMultipleTimesInCounty.map(shape => <span>{shape.PRECINCT_N} ({shape.PRECINCT_I}), </span>)}
      <br />
      <br />
      <Table
        columns={columns}
      ...
      {showCSVOutputs && (
        <Input.TextArea
          rows={4}
          value={convertToCSV(manualElectionResultsPrecinctsToShapeMap)}
        />
      )}
    </div>
  );
