...
import {
    Scatter, XAxis, YAxis, CartesianGrid, Tooltip, ZAxis, Line, ComposedChart, ScatterChart, ResponsiveContainer, ReferenceArea
} from 'recharts';
...
export default function VotesScatterPlot({ allElectionData, isCountyLevel, updateActiveHover, updateActiveSelection }) {
    ...
    const data = useMemo(() => {
        ...
        const regressionLineData = [{ x: maxX, y: regression.predict(maxX) }, { x: minX, y: regression.predict(minX) }];
    ...
    return <div style={{ width: "100%", height: "100%" }}>
        <div style={{ width: "100%", height: "100%" }}>
            <ResponsiveContainer width="100%" height="100%">
                <ComposedChart onMouseDown={handleMouseDown}
                    ...
                    <Scatter isAnimationActive={false}  name="2022 Absentee" fill="#000000" fillOpacity={0.5} data={data.pointsOnChart} onMouseEnter={hoverScatterDot} onMouseLeave={hoverScatterDotOut} onClick={clickScatterDot} />
