...
export default function VoteSummary({ geoJSONVote, activeSelection, updateCountySelected, updateActiveSelection, isCountyLevel, updateIsCountyLevel, updateUserHasSetLevel, electionResultCurrentElection, electionResultCurrentRace, electionResultBaseElection, electionResultBaseRace, absenteeElectionBaseID, showVoteMode, showDemographics, showAbsentee }) {
    ...
    return (<div>
        <h1>{resultSummary.CTYNAME ? resultSummary.CTYNAME : "The State of Georgia"} {resultSummary["PRECINCT_N"] ||
            ""}{" "}
            {activeSelection && (activeSelection?.properties?.id === geoJSONVote.properties.id || activeSelection === geoJSONVote.properties.id) && <span style={{ float: "right" }}><Button shape="circle" icon={<CloseOutlined />} onClick={() => {
        ...
        <Descriptions title={`${electionResultCurrentElection?.label} - ${electionResultCurrentRace?.name}`} column={1} bordered size="small" style={{ width: "100%" }} contentStyle={{ textAlign: "right", width: "40%" }}>
            ...
            <Descriptions.Item label="Vote Margin">{RDIndicator(resultSummary?.electionResultsCurrent?.marginDemocratic)} {numberFormat.format(Math.abs(resultSummary?.electionResultsCurrent?.marginRepublican))} ({numberFormatPercent.format(Math.abs(resultSummary?.electionResultsCurrent?.marginPerRepublican))})</Descriptions.Item>
            {resultSummary.electionResultsComparison && (<>
                <Descriptions.Item label="Shift in R/D %">{RDIndicator(resultSummary.electionResultsComparison.perShiftDemocratic)} {numberFormatPercent.format(Math.abs(resultSummary.electionResultsComparison?.perShiftDemocratic))}</Descriptions.Item>
                ...
                <Descriptions.Item label="Shift in Vote Margin">{RDIndicator(resultSummary.electionResultsComparison.voteShiftDemocratic)} {numberFormat.format(Math.abs(resultSummary.electionResultsComparison.voteShiftDemocratic))}</Descriptions.Item>
                <Descriptions.Item label="Normalized Shift in Vote Margin">{RDIndicator(resultSummary.electionResultsComparison.voteShiftDemocraticNormalized)}  {numberFormat.format(Math.abs(resultSummary.electionResultsComparison.voteShiftDemocraticNormalized))}</Descriptions.Item>
            </>)}
        </Descriptions>
        ...
        {resultSummary.electionResultsComparison && (<React.Fragment><br /><Descriptions title={`${electionResultBaseElection.label} - ${electionResultBaseRace.name}`} column={1} bordered size="small" style={{ width: "100%" }} contentStyle={{ textAlign: "right", width: "40%" }}>
            ...
            <Descriptions.Item label="Vote Margin">{RDIndicator(resultSummary?.electionResultsCurrent?.marginDemocratic)} {numberFormat.format(Math.abs(resultSummary?.electionResultsBase?.marginRepublican))} ({numberFormatPercent.format(Math.abs(resultSummary?.electionResultsBase?.marginPerRepublican))})</Descriptions.Item>
        ...
        {showDemographics && resultSummary?.demographics?.whitePer > 0 && (<Descriptions title="Demographics of 2020 Registered Voters " column={1} bordered size="small" style={{ width: "100%" }} contentStyle={{ textAlign: "right", width: "40%" }}>
        ...
        {showAbsentee && (<Descriptions title="Absentee Ballots" column={1} bordered size="small" style={{ width: "100%" }} contentStyle={{ textAlign: "right", width: "40%" }}>
